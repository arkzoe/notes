...
全局守卫是注册在 `router` 实例上的，对所有路由生效，常用的有：

- `router.beforeEach`：路由跳转**前**触发（最常用）
- `router.afterEach`：路由跳转**后**触发（无拦截能力）
```js
import { createRouter, createWebHistory } from 'vue-router'
...
const routes = [
  ...
  { path: '/profile', name: 'Profile', component: () => import('../views/Profile.vue'), meta: { requiresAuth: true } } // 需要登录的页面
]
...
直接在路由配置中定义，只对当前路由生效，常用 `beforeEnter`
```js
const routes = [
  {
    path: '/admin',
...
在组件内部定义，只对当前组件生效，常用的有：

- `onBeforeRouteEnter`：进入组件前触发（此时组件实例还未创建，无法访问 `this`）
- `onBeforeRouteUpdate`：路由参数变化时触发（比如从 `/user/1` 跳转到 `/user/2`）
- `onBeforeRouteLeave`：离开组件前触发（比如提示用户 “未保存的内容会丢失”）
```vue
<template>
  <div>用户详情页</div>
</template>
...
onBeforeRouteLeave((to, from, next) => {
  const confirmLeave = confirm('确定要离开吗？未保存的内容会丢失！')
  if (confirmLeave) {
    next()
  } else {
    next(false) // 取消跳转
...
3. 示例：
   ...
   ```js
   vonst emit = defineEmits(['send-toy']) 
   //子组件中，触发事件：
...
|指令|作用|示例|
|---|---|---|
|`v-text`|渲染文本（替代插值 `{{}}`，不会解析 HTML）|`<div v-text="message"></div>`|
|`v-html`|渲染带 HTML 的内容（注意 XSS 风险）|`<div v-html="htmlContent"></div>`|
|`v-show`|基于布尔值控制元素显示 / 隐藏（DOM 仍存在，修改 `display`）|`<div v-show="isShow">显示/隐藏</div>`|
|`v-if`|基于布尔值条件渲染元素（DOM 会创建 / 销毁）|`<div v-if="isExist">存在则显示</div>`|
|`v-else-if`|`v-if` 的分支条件|`<div v-else-if="type === 'b'">B</div>`|
|`v-else`|`v-if` 的兜底条件|`<div v-else>其他</div>`|
|`v-for`|循环渲染列表（Vue3 要求绑定 `key`）|`<li v-for="item in list" :key="item.id">{{item.name}}</li>`|
|`v-on`|绑定事件（简写 `@`）|`<button @click="handleClick">点击</button>`|
|`v-bind`|绑定属性（简写 `:`）|`<img :src="imgUrl" :class="cls">`|
|`v-model`|双向数据绑定（支持表单元素）|`<input v-model="username">`|
|`v-slot`|定义插槽（简写 `#`）|`<Child #default>默认插槽</Child>`|
|`v-pre`|跳过编译（显示原始插值语法）|`<div v-pre>{{ 不会被编译 }}</div>`|
|`v-cloak`|解决插值闪烁（配合 CSS：`[v-cloak] { display: none }`）|`<div v-cloak>{{ message }}</div>`|
|`v-once`|只渲染一次（数据变化不再更新）|`<div v-once>{{ onceMsg }}</div>`|
